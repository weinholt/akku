     ...
     (lambda (dep)
       (let-values (((package-name range)
         ...
         (let ((package-name (parse-package-name package-name)))
           (unless (add-dependency manifest-filename (get-index-filename)
...
(define (cmd-publish arg* opts)
  ...
  (let ((version-override (assq-ref opts 'version #F))
        (tag-override (assq-ref opts 'tag #f)))
...
(define (cmd-dependency-scan arg* opts)
  ...
  (let ((impl* (cond ((assq-ref opts 'implementation #f)
